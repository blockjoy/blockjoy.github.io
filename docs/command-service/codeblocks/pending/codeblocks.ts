...
import { CommandServicePendingRequest, CommandServicePendingResponse } from "api/sdk/ts/blockjoy/v1/command_pb"

var commandService0 = new CommandServiceClient('https://unknown-grpc-web')
function App(){
    const [response, setResponse] = useState<string>("")
    useEffect(() => { 
        const commandservicependingrequest = new CommandServicePendingRequest()
        
        commandService0.pending(commandservicependingrequest, getMetadata(), (err, value:CommandServicePendingResponse|null) => {
...
const typescriptCodeShorten = `const commandservicependingrequest = new CommandServicePendingRequest();

service.pending(commandservicependingrequest, (err, value:CommandServicePendingResponse|null) => {
...
func main() {
   ...
   r1, _ := commandService.Pending(ctx, &v1.CommandServicePendingRequest{})
   fmt.Println(r1)
}
...
const goCodeShorten = `resp, _ := commandService.Pending(ctx, &v1.CommandServicePendingRequest{})
fmt.Println(resp)`;
...
function App(){
    ...
    useEffect(() => { 
        ...
        commandService0.pending(commandservicependingrequest, getMetadata(), (err, value) => {
            setResponse(JSON.stringify(err ? err : value))
        })
    }, [])
    return <>
...
const javascriptCodeShorten = `const commandservicependingrequest = new CommandServicePendingRequest();

service.pending(commandservicependingrequest, (err, value) => {
    const resp = JSON.stringify(err ? err : value);
    console.log("received ", resp);
})`;
...
use api::blockjoy::v1::CommandServicePendingRequest;
...
// sending request and waiting for response
  let response = client.pending(request).await?.into_inner();
